...
class TranslationService extends ChangeNotifier {
  ...
  bool get isReady => _currentLocale == 'en' || _translations.isNotEmpty;

  // 翻訳済みのキー数（PostDownloadSetupScreen の進捗表示用）
  int get translatedCount => _translations.length;

  static const _cacheKeyPrefix = 'ui_translations_v1_';
  ...
  static const Map<String, String> englishStrings = {
    ...
    'home_questionnaire': 'Fill out questionnaire',
    ...
    // ─ 期間 (6) ─（過去のことなので「今すぐ→今・直前」のような誤訳を避ける）
    'duration_now': 'A moment ago (just now)',
    'duration_hours': 'A few hours ago',
    'duration_today': 'Earlier today',
    'duration_yesterday': 'Since yesterday',
    'duration_days': 'A few days ago',
    'duration_week': '1 week or more ago',
    ...
    'result_summary_redo_hint':
        'If this is wrong, tap "Re-do" below.',
  ...
  Future<void> ensureTranslated() async {
    ...
    try {
      // 100 キーを単発で翻訳すると不安定なので、~25 キーずつチャンク分割
      // 各チャンク完了ごとに notifyListeners → UI が徐々に翻訳されていく
      const chunkSize = 25;
      final allKeys = englishStrings.keys.toList();
      // 既に翻訳済みのキーはスキップ（再開時に重複翻訳しない）
      ...
      debugPrint(
          '[TranslationService] Translating ${pending.length}/${allKeys.length} keys to "$_currentLocale" in chunks of $chunkSize...');
      ...
      for (var i = 0; i < pending.length; i += chunkSize) {
        ...
        final chunkMap = <String, String>{
          for (final k in chunkKeys) k: englishStrings[k]!,
        };

        try {
          final result = await GemmaService.translateUiStrings(
            chunkMap,
            _currentLocale,
          );
          if (result != null && result.isNotEmpty) {
            _translations.addAll(result);
            // 進捗をキャッシュに保存（途中で落ちても次回再開できる）
            final prefs = await SharedPreferences.getInstance();
            ...
            debugPrint(
                '[TranslationService] Chunk ${(i ~/ chunkSize) + 1}: +${result.length} keys (total ${_translations.length}/${allKeys.length})');
            notifyListeners();
          } else {
            debugPrint(
                '[TranslationService] Chunk ${(i ~/ chunkSize) + 1} returned empty.');
          }
        } catch (e) {
          debugPrint(
              '[TranslationService] Chunk ${(i ~/ chunkSize) + 1} failed: $e');
          // チャンク単位でリトライせず次へ（部分翻訳でも UI は動く）
        }
      }

      debugPrint(
          '[TranslationService] Done. ${_translations.length}/${allKeys.length} keys translated.');
    } catch (e) {
